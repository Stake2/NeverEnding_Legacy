...
func:function()
{
	/*
		...
		Note : Warning, Homo sapient legacy author thinks huamn history is awesome.
	*/
	...
	G.funcs['new game']=function()
	{
		...
		G.Message({type:'important tall',text:str,icon:[0,3]})
		{
		 ...
		 {G.gainTech(G.techByName['mausoleum complete']),G.middleText('You know where you are.<br>And this is not after life.<br>You started to question it.','slow')}
		 }
	};
	...
	G.resCategories={
		...
		'demog':{
			...
			side:['population','housing','building slot','corpse','burial spot'],
		},
		'food':{
			name:'Food & Water',
			base:[],
	...
	new G.Res({
		...
		tick:function(me,tick)
		{
			...
			if (me.amount>0)
			{
				...
				if (tick%50==0)
				{
					var rituals=['mating gathering','flower rituals','wisdom rituals'];
				...
				//fire
				var objects={'fire pit':[10,0.1,0.1],'torch':[5,0.1,0.1]};
				var leftout=me.amount;
				var prev=leftout;
				var fulfilled=0;
				for (var i in objects)
	...
	new G.Unit({
		...
		modes:{
			...
			'gather archaic materials':{name:'gather archaic materials',icon:[2,7],desc:'Ingore food and water,only gather materials'},
			'gather rare herb':{name:'gather rare herbs',icon:[4,6],desc:'Dedicate themselves to find strange herbs with strange properties, such as healing, using [basket]s as tool.',req:{'plant lore':true}},
		},
		effects:[
			{type:'gather',context:'foodgather',amount:2,max:4,mode:'gather food only'},
			{type:'gather',context:'watergather',what:{'water':1,'muddy water':4},amount:3,max:6,mode:'gather water only'},
			...
			{type:'gather',context:'materialgather',what:{'stick':0.5,'stone':0.5,'mud':0.5},amount:2,max:2,mode:'gather archaic materials',req:{'scavenging':true}},
	...
	new G.Unit({
		...
		gizmos:true,
		modes:{
			'stick fires pit':{name:'Start fires from sticks',icon:[0,6,13,7],desc:'Craft [fire pit]s from 12 [stick]s each.'},
			'make torches':{name:'Start fires from sticks and some herbs',icon:[0,6,13,7],desc:'Craft [torch]s from 5 [stick]s each.'},
		...
		effects:[
			{type:'convert',from:{'stick':12},into:{'fire pit':1},every:5,mode:'stick fires pit'},
	...
	new G.Unit({
		name:'city',
		desc:'@provides 2000 [housing]@provides 100 [building slot]s<>A tile full of human stuctures.',
		icon:[8,14,'H1sheet'],
	...
	new G.Unit({
		...
		modes:{
			...
			'recuit wild human':{name:'Recuit wild human',desc:'[wanderer]s will try to intergrate any wild man they can communicate into our tribe, they use up [culture] and [food]s in this process.',req:{'chieftains':true}},
		},
		effects:[
	...
	new G.Tech({
		name:'speech',
		desc:'@unlocks [wanderer]@provides 10 [wisdom]<>[speech], in its most primitive form, is a series of groans and grunts that makes it possible to communicate things, events, and concepts.',
		icon:[1,1],
		startWith:true,
		effects:[
	...
	new G.Tech({
		name:'use of tool',
		desc:'@unlocks [gatherer]s mode to gather [archaic building materials], humanity realised tool usage, they pick up simple stone and sticks to break.',
		icon:[31,2,'H1sheet'],
		startWith:true,
		effects:[
		],
	...
	new G.Tech({
		name:'language',
		desc:'@[chieftain] generates more insights@provides 25 [inspiration]@provides 15 [wisdom]<>[language] improves on [speech] by combining complex grammar with a rich vocabulary, allowing for better communication and the first signs of culture.',
		icon:[2,1],
		cost:{'insight':1},
		req:{'speech':true},
		effects:[
			{type:'provide res',what:{'inspiration':25,'wisdom':15}},
		],
		chance:50,
	});
	...
	new G.Tech({
		name:'oral tradition',
		desc:'@unlocks [dreamer]@unlocks [storyteller]@provides 20 [inspiration]@provides 25 [wisdom]<>[oral tradition] emerges when the members of a tribe gather at night to talk about their day. Stories, ideas, and myths are all shared and passed on from generation to generation.',
		icon:[5,1],
		cost:{'insight':10},
		req:{'language':true},
		effects:[
			{type:'provide res',what:{'inspiration':20,'wisdom':25}},
		],
		chance:50,
	});
	...
	new G.Tech({
		...
		icon:[8,1],
		cost:{'insight':15},
		req:{'use of tool':true,'digging':true,'language':true},
		effects:[
		],
		chance:3,
	});
	...
	new G.Tech({
		...
		icon:[22,8],
		cost:{'insight':25},
		req:{'construction':true,'cities':true,'basic drawing':true},
		effects:[
		],
	});
	new G.Tech({
	...
	new G.Tech({
		...
		icon:[16,1],
		cost:{'insight':15},
		req:{'stone-knapping':true,'discovery of fire':true},
		effects:[
		],
		chance:3,
	});
	...
	new G.Tech({
        ...
        icon:[25,8,'H1sheet'],
        cost:{'insight':50},
        req:{'cities':true,'clans':true,'basic drawing':true},
        effects:[
        ],
    });
	...
	new G.Trait({
		...
		desc:'@people ate too much ration, and they learn that eating excessively is wrong, and is unhealthy. @people consume 15% less [food], but derive less joy from eating',
		icon:[3,12,19,1],
		cost:{'culture':5},
		chance:50,
		req:{'tribalism':true,'joy of eating':false,'food rations':'plentiful'},
	});
	new G.Trait({
		name:'joy of eating',
		desc:'@people ate little and suffered, and they learn that eating is fairly enjoyable, food makes strong men. people consume 15% more [food], and are happier when eating',
		icon:[4,12,19,1],
		cost:{'culture':5},
		chance:50,
		req:{'tribalism':true,'culture of moderation':false,'food rations':'Meager'},
	});
	new G.Trait({
		name:'insect-eating',
	...
	new G.Policy({
		...
		desc:'Improves birth rate by 20%. Consumes 1 [faith] every 20 days; will stop if you run out.<>A common practise in many cultrue is to gather up all of their teenagers to have a big festival, and put them together. This is one of those gathering as well.',
		icon:[8,12,2,3],
	...
	new G.Policy({
		name:'wisdom rituals',
		desc:'Improves [dreamer] and [storyteller] efficiency by 20%. Consumes 2 [faith] every 20 days; will stop if you run out.',
		icon:[8,12,8,5],
		cost:{'faith':2},
		startMode:'off',
		req:{'ritualism':true},
	...
	new G.Policy({
		...
		desc:'Improves [gatherer], [hunter] and [fisher] efficiency by 20%. Consumes 5 [influence] every 20 days; will stop if you run out.',
		icon:[4,12,4,7],
		cost:{'influence':5},
		startMode:'off',
		req:{'clans':true},
	...
	new G.Land({
		...
		goods:[
			{type:'fish',min:1,max:4},
			{type:'avians',chance:0.1,amount:0.1},
			{type:'costal critters',chance:0.4,min:1,max:4},
			{type:'saltwater'},
		],
		ocean:true,
		image:4,
	...
	new G.Land({
		...
		goods:[
			...
			{type:'grass',amount:2},
			{type:'forest critters',chance:0.9},
			{type:'large mammal predators',chance:0.2,amount:0.5},
			{type:'large mammal preys',chance:0.2,amount:0.2},
			{type:'wild bugs'},
			{type:'fish',chance:0.8,min:0.1,max:0.5},
			{type:'costal critters',chance:0.1,min:0.1,max:0.3},
			{type:'freshwater',amount:1},
			{type:'avians',chance:0.9,amount:0.5,amount:0.2},
	...
	new G.Land({
		...
		goods:[
			...
			{type:'forest critters',chance:0.6},
			{type:'large mammal predators',chance:0.1,amount:0.2},
			{type:'avians',chance:0.4,amount:0.9},
			{type:'wild bugs'},
			{type:'fish',chance:0.3,min:0.1,max:0.3},
	...
	new G.Land({
		...
		goods:[
			...
			{type:'fish',chance:0.1,min:0.1,max:0.3},
			{type:'costal critters',chance:0.05,min:0.1,max:0.3},
			{type:'freshwater',amount:1},
			{type:'rocky substrate'},
		],
	...
	new G.Land({
		...
		goods:[
			...
			{type:'large mammal preys',chance:0.2,amount:0.1},
			{type:'avians',chance:0.3,amount:0.3},
			{type:'wild bugs'},
			{type:'fish',chance:0.8,min:0.1,max:0.5},
			{type:'costal critters',chance:0.05,min:0.1,max:0.3},
	...
	new G.Land({
		...
		goods:[
			...
			{type:'avians',chance:0.3,amount:0.3},
			{type:'wild bugs'},
			{type:'fish',chance:0.6,min:0.1,max:0.5},
			{type:'costal critters',chance:0.05,min:0.1,max:0.3},
			{type:'freshwater',amount:0.8},
	...
	new G.Goods({
		...
		icon:[10,10],
		res:{
			'foodgather':{'herb':6,'fruit':0.5,'vegetable':2},
	...
	new G.Goods({
		name:'avians',
		desc:'[avians] are also critters, they flys high from any danger, tho there are also some decent walkers among them, tho hunting them down can yield nice amount of [meat] and [bone]s. Consist of Birds of all kinds,pheasant of all kinds, ostriches and many others.//their nests can be a source for gathering [egg].',
		icon:[17,11,'H1sheet'],
		res:{
			'foodgather':{'egg':1},
	...
	new G.Goods({
		...
		icon:[13,10],
		res:{
			'watergather':{'water':4,'muddy water':8},
			'dig':{'ice':0.2},
		},
		mult:5,
	});
	...
	new G.Goods({
		...
		icon:[15,10],
		res:{
			'watergather':{'water':8,'muddy water':8},
		},
		mult:5,
	});
